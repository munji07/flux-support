...
async function ensureHouseTables(db) {
  ...
  await db
    .query(
      `CREATE INDEX IF NOT EXISTS idx_houses_guild_owner ON dishouse_houses(guild_id, owner_id)`,
    )
    .catch(() => {});
  await db
    .query(
      `CREATE INDEX IF NOT EXISTS idx_houses_channel ON dishouse_houses(channel_id)`,
    )
    .catch(() => {});
}
...
async function getNextFloor(db, guildId) {
  ...
  const { rows } = await db.query(
    `SELECT COALESCE(MAX(floor), 4) + 1 AS next_floor FROM dishouse_houses WHERE guild_id=$1`,
    [guildId],
  );
...
async function getOrCreateCategory(guild) {
  ...
  const existing = guild.channels.cache.find(
    (c) =>
      c.type === ChannelType.GuildCategory && c.name === HOUSE_CATEGORY_NAME,
  );
  if (existing) return existing;
  const fetched = await guild.channels.fetch().catch(() => null);
  if (fetched) {
    const cat = [...fetched.values()].find(
      (c) =>
        c.type === ChannelType.GuildCategory && c.name === HOUSE_CATEGORY_NAME,
    );
    if (cat) return cat;
...
async function createHouseChannel(
  guild,
  floor,
  displayName,
  ownerId,
  ...
  const ch = await guild.channels.create({
    ...
    topic: isPublic
      ? `${displayName}님의 공용 하우스 — 누구나 입장 가능`
      : `${displayName}님의 개인 하우스 — 초대받은 사람만 입장 가능`,
  });
  return ch;
}
...
async function updateHouseChannelPermissions(guild, house) {
  ...
  try {
    ...
    console.warn("[houses] update permissions", e.message);
  }
}
...
async function ensureHouse(db, guild, ownerId, displayName) {
  ...
  if (rows[0]?.channel_id) {
    ...
    await db.query(
      `UPDATE dishouse_houses SET channel_id=$1, channel_name=$2, owner_name=$3, updated_at=now() WHERE id=$4`,
      [ch2.id, ch2.name, displayName, rows[0].id],
    );
  ...
  if (rows[0] && !rows[0].channel_id) {
    ...
    await db.query(
      `UPDATE dishouse_houses SET channel_id=$1, channel_name=$2, owner_name=$3, updated_at=now() WHERE id=$4`,
      [ch.id, ch.name, displayName, rows[0].id],
    );
...
async function setVisibility(db, guildId, ownerId, visibility) {
  if (!db) throw new Error("DATABASE_URL 없음");
  if (!Object.values(HOUSE_VISIBILITY).includes(visibility))
    throw new Error("visibility 오류");
  const { rows } = await db.query(
    `UPDATE dishouse_houses SET visibility=$3, updated_at=now() WHERE guild_id=$1 AND owner_id=$2 RETURNING *`,
    [guildId, ownerId, visibility],
  );
  return rows[0] ?? null;
}
...
async function listInvites(db, houseId) {
  if (!db) return [];
  const { rows } = await db.query(
    `SELECT target_id, invited_by, created_at FROM dishouse_house_invites WHERE house_id=$1 ORDER BY created_at`,
    [houseId],
  );
  return rows;
}
...
async function canAccess(db, guildId, ownerId, viewerId) {
  ...
  const cleanViewer = String(viewerId)
    .replace(/[<@!>]/g, "")
    .trim();
