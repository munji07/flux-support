...
function createBalanceGame({
  ...
  async function generateBalanceQuestion() {
    const fallbacks = [
      {
        title: "⚖️ 평생 하나만 먹어야 한다면?",
        optionA: "평생 치킨만 먹기",
        optionB: "평생 피자만 먹기",
      },
      {
        title: "⚖️ 진짜 기괴한 능력 딜레마",
        optionA: "소리 지를 때마다 10만 원 나오기 (귀 찢어지게 지름)",
        optionB: "조용히 눈 깜빡일 때마다 1천 원 나오기",
      },
      {
        title: "⚖️ 주말 극악의 딜레마",
        optionA: "하루 종일 집에 혼자 있고 아무도 연락 안 옴",
        optionB: "하루에 약속 5개 잡혀서 쉬지도 못함",
      },
      {
        title: "⚖️ 폰 딜레마",
        optionA: "평생 Wi-Fi / 데이터 없이 살기",
        optionB: "평생 에어컨 / 히터 없이 살기",
      },
      {
        title: "⚖️ 친구 / 연애 딜레마",
        optionA: "내 흑역사 다 아는 찐친",
        optionB: "나한테 비밀이 전혀 없는 연인",
      },
      {
        title: "⚖️ 수면 딜레마",
        optionA: "하루 3시간 자고 완전 피곤함 제로",
        optionB: "하루 12시간 자야만 겨우 안 피곤함",
      },
      {
        title: "⚖️ 민트초코 딜레마",
        optionA: "평생 모든 음식에서 민트맛 나기",
        optionB: "평생 모든 음식에서 탄맛 나기",
      },
    ];
    ...
    try {
      ...
      if (error.name !== "AbortError") {
        console.warn(
          "[BalanceGame] AI 질문 생성 실패, 템플릿 질문을 사용합니다:",
          error.message,
        );
      }
    }
    ...
    try {
      ...
      if (channel?.isTextBased()) {
        ...
        const embed = new EmbedBuilder()
          ...
          .addFields({
            name: "📊 실시간 투표 현황",
            value: "🅰️ 0표 (0%) vs 🅱️ 0표 (0%)\n*(총 0명 참여)*",
          })
          .setFooter({
            text: "버튼을 눌러 투표하세요! 언제든지 선택을 변경할 수 있습니다.",
          })
          .setTimestamp();

        const row = new ActionRowBuilder().addComponents(
          new ButtonBuilder()
            .setCustomId(`balance:A:${gameId}`)
            .setLabel(`🅰️ A 선택`)
            .setStyle(ButtonStyle.Primary),
          new ButtonBuilder()
            .setCustomId(`balance:B:${gameId}`)
            .setLabel(`🅱️ B 선택`)
  ...
  async function handleButtonInteraction(interaction) {
    ...
    const percentA =
      totalVotes > 0 ? Math.round((gameData.votesA / totalVotes) * 100) : 0;
    const percentB =
      totalVotes > 0 ? Math.round((gameData.votesB / totalVotes) * 100) : 0;
